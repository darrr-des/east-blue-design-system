  ...
  "style": {
    ...
    "colorsTables": [
      buildColorsTable({
        ...
        rows: [
          { role: "Surface", token: "bg/color-bg-inverse-weaker · onLight bg/color-bg-brand · warning and info steps by state",
  ...
  "changelog": [
    {
      ...
      "kind": "patch",
      "kindLabel": "Patch",
      "header": "Surface token corrected — node 5630:36052",
      "rows": [
        {
          "body": "<strong>The Surface was documented against the wrong token.</strong> <code>#EEF2F9</code> was recorded as <code>bg/color-bg-secondary</code> on all three cards and in the colours table; it is <code>bg/color-bg-inverse-weaker</code>. The hex was always right — only the name was wrong, which is the worse of the two failures, because a developer implements the name. Caught while reviewing <a href=\"/components/dropdown-item\">Select Item</a>, whose badge is genuinely <code>bg/color-bg-secondary</code> at <code>#E5F1FF</code>: one token cannot be two values, and comparing the two pages is what surfaced it.",
